...
import MissingDaysManager from '../components/MissingDaysManager';
import SecurityQuestionSetup from '../components/SecurityQuestionSetup';
import { updateDailyRecord, getTodayDate } from '../../lib/savingsManager';
import { needsMigration, migrateUserToNewSystem, needsSecurityQuestion } from '../../lib/migrationService';
...
const App: React.FC = () => {
    ...
    // إدارة الترحيل والأسئلة الأمنية
    const handleUserMigrationAndSecurity = useCallback(async () => {
        try {
            // فحص الحاجة للترحيل
            const userNeedsMigration = await needsMigration(username);
            if (userNeedsMigration) {
                setIsMigrating(true);
                showNotification('جارٍ ترحيل بياناتك إلى النظام الجديد...', 'info');
                
                await migrateUserToNewSystem(username);
                showNotification('تم ترحيل بياناتك بنجاح!', 'success');
            ...
            // فحص الحاجة للسؤال الأمني
            const userNeedsSecurityQuestion = await needsSecurityQuestion(username);
            if (userNeedsSecurityQuestion) {
                setShowSecurityQuestionSetup(true);
            }
        } catch (error) {
            console.error('Migration or security check error:', error);
            showNotification('حدث خطأ في فحص البيانات', 'error');
        }
    }, [username]);
    ...
    useEffect(() => {
        ...
        const fetchUser = async () => {
            try {
                ...
                if (userSnap.exists()) {
                    ...
                    // فحص الترحيل وإدارة الأسئلة الأمنية
                    await handleUserMigrationAndSecurity();
        ...
        fetchUser();
    }, [username, handleUserMigrationAndSecurity]);
    ...
    const handleDailyQuestionAnswer = async (didSmoke: boolean) => {
        ...
        try {
            // استخدام نظام المدخرات الجديد
            await updateDailyRecord(username, today, didSmoke);
            ...
            if (userSnap.exists()) {
                const updatedData = userSnap.data() as UserData;
                setUserData(updatedData);
                
                if (typeof window !== 'undefined') {
                    localStorage.setItem('anfask-userData-' + username, JSON.stringify(updatedData));
                }
            }
            
            if (didSmoke) {
                showNotification('تم تسجيل اليوم. تذكر أن كل يوم جديد هو فرصة للبدء من جديد!', 'warning');
            } else {
                showNotification('أحسنت! تم تسجيل يوم آخر بدون تدخين 🎉', 'success');
            }
    ...
    return (
        <>
            ...
            {/* نظام تسجيل الأيام المفقودة */}
            {username && <MissingDaysManager username={username} />}

            {/* إعداد السؤال الأمني للمستخدمين القدامى */}
            {showSecurityQuestionSetup && (
                <SecurityQuestionSetup
                    username={username}
                    onComplete={() => {
                        setShowSecurityQuestionSetup(false);
                        showNotification('تم حفظ السؤال الأمني بنجاح!', 'success');
                    }}
                    onSkip={() => {
                        setShowSecurityQuestionSetup(false);
                        showNotification('يمكنك إضافة السؤال الأمني لاحقاً من الإعدادات', 'info');
                    }}
                />
            )}
